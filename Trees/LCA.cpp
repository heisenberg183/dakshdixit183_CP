...
// Code and Explanation by Errichto 
...
void solve(){
    int n,q;
    cin>>n>>q;
    adj.clear();adj.resize(n);//n+1
    up.clear();up.resize(n,vector<int>(LOG,-1));//n+1
    ht.clear();ht.resize(n,0);//n+1
    for(int i=1;i<n;i++){
        int j;
        cin>>j;
        j--;
    ...
    //Kth Ancestor and LCA precomputation 
    dfs(0);//n+1
    for(int bit=1;bit<LOG;bit++){
        for(int v=0;v<n;v++){//n+1
            if(up[v][bit-1]!=-1){
